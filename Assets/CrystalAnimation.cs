...
public class CrystalAnimation : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isExpansion)
        {
            ...
            transform.localScale = new Vector3(Mathf.Clamp(transform.localScale.x + Time.deltaTime, 0, 3), transform.localScale.y, Mathf.Clamp(transform.localScale.z + Time.deltaTime, 0, 3));
        }
        //縮小
        if (isReduction)
        {
